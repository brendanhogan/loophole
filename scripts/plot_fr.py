...
ax.annotate("learned attacker hardens\nthe filter almost immediately",
            xy=(2, 0.27), xytext=(4.4, 0.66), fontsize=11, color=SMART,
...
ax.set_xlabel("training round"); ax.set_ylabel("overall accuracy"); ax.set_ylim(0.86, 1.005)
title(ax, "The filter gets accurate faster", "balanced allow/block accuracy · mean ± std over 5 seeds")
...
for mode in ("grpo", "greedy"):
    ...
    if a is None:
        continue
    t = [(np.where(r <= THRESH)[0][0] if np.any(r <= THRESH) else a.shape[1] - 1) for r in a]
...
for mode in ("grpo", "greedy"):
    ...
    if a is not None:
        band(ax, mode, np.maximum.accumulate(a, axis=1), marker="^")
